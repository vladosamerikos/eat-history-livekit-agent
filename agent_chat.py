"""
LiveKit conversational voice agent for Eat History ("coach").

Segundo agente, independiente del agente clásico `eat-history`. Se registra como
worker con agentName="eat-history-chat" y se despacha desde el backend con
metadata configurable:
    {
      "userId": str,
      "locale": "es" | "en" | "uk",
      "mcpToken": <JWT corto para llamar al MCP>,
      "apiBase":  "https://eat-history.vladys.dev/v1",
      "model":    "gemini-2.5-flash" | "gemini-2.5-pro" | ...,   # opcional
      "ttsEngine": "chirp3" | "journey" | "neural2"               # opcional
    }

A diferencia del primer agente, el MODELO LLM y la VOZ (TTS) se pueden
personalizar por context params (y, por tanto, desde la propia app del usuario).

- STT: Google Cloud Speech-to-Text (streaming)
- TTS: Google Cloud Text-to-Speech (motor configurable)
- LLM: Vertex Gemini (modelo configurable) vía google-genai con vertexai=True
- Tools: las del MCP server en /v1/mcp (HTTP streamable) usando el mcpToken.
"""
from __future__ import annotations

import asyncio
import json
import logging
import os
from dataclasses import dataclass
from typing import Any

from livekit import agents
from livekit.agents import (
    Agent,
    AgentSession,
    JobContext,
    JobProcess,
    RoomInputOptions,
    WorkerOptions,
    mcp,
    llm,
)
from livekit.plugins import google, silero

import config

logger = logging.getLogger("eat-history-chat-agent")
logging.basicConfig(level=os.getenv("LOG_LEVEL", "INFO"))

AGENT_NAME = "eat-history-chat"


def load_prompts_for_locale(locale: str) -> tuple[str, str, str]:
    """Carga los prompts conversacionales (chat_*.txt) para el locale dado."""
    supported_locales = ["es", "en", "uk"]
    loc = locale if locale in supported_locales else "es"

    current_dir = os.path.dirname(os.path.abspath(__file__))
    prompt_path = os.path.join(current_dir, "prompts", f"chat_{loc}.txt")

    system_prompt = (
        "Eres el coach nutricional por voz de Eat History. Ayuda al usuario "
        "con su alimentación y registra comidas y peso cuando lo pida."
    )
    greeting_prompt = "Genera un saludo inicial amigable."
    greeting_fallback = "Hola, soy tu coach. ¿En qué te ayudo hoy?"

    try:
        with open(prompt_path, "r", encoding="utf-8") as f:
            content = f.read()

        if "=== GREETING_FALLBACK ===" in content:
            parts_fallback = content.split("=== GREETING_FALLBACK ===")
            greeting_fallback = parts_fallback[1].strip()
            content = parts_fallback[0]

        if "=== GREETING_PROMPT ===" in content:
            parts_greeting = content.split("=== GREETING_PROMPT ===")
            greeting_prompt = parts_greeting[1].strip()
            system_part = parts_greeting[0].replace(
                "=== SYSTEM_PROMPT ===", "").strip()
            if system_part:
                system_prompt = system_part
        else:
            system_prompt = content.replace(
                "=== SYSTEM_PROMPT ===", "").strip()
    except Exception as e:
        logger.error("Error loading chat prompts for %s: %s", loc, e)

    return system_prompt, greeting_prompt, greeting_fallback


@dataclass
class JobMeta:
    user_id: str
    locale: str
    mcp_token: str
    api_base: str
    model: str
    tts_engine: str | None

    @classmethod
    def parse(cls, raw: str | None) -> "JobMeta":
        data: dict[str, Any] = json.loads(raw) if raw else {}
        return cls(
            user_id=data.get("userId", "unknown"),
            locale=data.get("locale", "es"),
            mcp_token=data.get("mcpToken", ""),
            api_base=data.get("apiBase", "https://eat-history.vladys.dev/v1"),
            model=(data.get("model") or config.LLM_MODEL),
            tts_engine=(data.get("ttsEngine") or None),
        )


def prewarm(proc: JobProcess) -> None:
    """Preload VAD model so the first request is fast."""
    proc.userdata["vad"] = silero.VAD.load()


async def entrypoint(ctx: JobContext) -> None:
    meta = JobMeta.parse(ctx.job.metadata)
    logger.info(
        "chat job start user=%s locale=%s model=%s tts=%s room=%s",
        meta.user_id,
        meta.locale,
        meta.model,
        meta.tts_engine or config.TTS_ENGINE,
        ctx.room.name,
    )

    await ctx.connect()

    stt_lang = config.STT_LANG.get(meta.locale, "es-ES")
    tts_lang, tts_voice = config.get_tts_voice(meta.locale, meta.tts_engine)

    instructions, greeting_prompt, greeting_fallback = load_prompts_for_locale(
        meta.locale)

    @llm.function_tool(
        description="Ends the call/hangs up. Call this when the user says they are done, want to say goodbye, or want to hang up."
    )
    def end_call() -> str:
        logger.info("Ending chat call per user request")
        asyncio.create_task(ctx.disconnect())
        return "Ending the call. Goodbye!"

    session = AgentSession(
        vad=ctx.proc.userdata["vad"],
        stt=google.STT(
            languages=[stt_lang],
            model="latest_long",
        ),
        llm=google.LLM(
            model=meta.model,
            vertexai=True,
            project=os.getenv("GOOGLE_VERTEX_PROJECT"),
            location=os.getenv("GOOGLE_VERTEX_LOCATION", "us-central1"),
        ),
        tts=google.TTS(language=tts_lang, voice_name=tts_voice),
        tools=[end_call],
        mcp_servers=[
            mcp.MCPServerHTTP(
                url=f"{meta.api_base}/mcp",
                headers={"Authorization": f"Bearer {meta.mcp_token}"},
                timeout=15,
                client_session_timeout_seconds=30,
            ),
        ],
    )

    await session.start(
        room=ctx.room,
        agent=Agent(instructions=instructions),
        room_input_options=RoomInputOptions(),
    )

    # Saludo inicial dinámico generado por el LLM.
    try:
        greeting_ctx = llm.ChatContext()
        greeting_ctx.add_message(role="user", content=greeting_prompt)

        stream = session.llm.chat(chat_ctx=greeting_ctx)
        greeting_text = ""
        async for chunk in stream:
            if chunk.delta and chunk.delta.content:
                greeting_text += chunk.delta.content
        greeting_text = greeting_text.strip()
    except Exception as e:
        logger.error("Error generating dynamic greeting: %s", e)
        greeting_text = greeting_fallback

    await session.say(greeting_text, allow_interruptions=True)


if __name__ == "__main__":
    agents.cli.run_app(
        WorkerOptions(
            entrypoint_fnc=entrypoint,
            prewarm_fnc=prewarm,
            agent_name=AGENT_NAME,
        )
    )
